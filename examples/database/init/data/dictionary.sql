WITH space AS
     ...
     user_ids AS
       (SELECT id, username
        FROM ncse_sys_user)
INSERT
INTO ncse_sys_dictionary (id, name, slug, type, value, space_id, created_by, created_at, updated_by, updated_at)
VALUES
  -- 系统状态
  (nanoid(), '通用状态', 'common_status', 'object', '{"0":"禁用","1":"启用"}', (SELECT id FROM space),
  ...
  (nanoid(), '审核状态', 'audit_status', 'object', '{"0":"待审核","1":"审核通过","2":"审核拒绝"}',
   (SELECT id FROM space), (SELECT id FROM user_ids WHERE username = 'admin'),
  ...
  (nanoid(), '通用优先级', 'common_priority', 'object', '{"1":"低","2":"中","3":"高","4":"紧急"}',
   ...
   EXTRACT(EPOCH FROM now()) * 1000 + floor(random() * 1000), (SELECT id FROM user_ids WHERE username = 'admin'),
   EXTRACT(EPOCH FROM now()) * 1000 + floor(random() * 1000)),
  (nanoid(), '任务优先级', 'task_priority', 'object', '{"1":"P0","2":"P1","3":"P2","4":"P3"}', (SELECT id FROM space),
   (SELECT id FROM user_ids WHERE username = 'admin'), EXTRACT(EPOCH FROM now()) * 1000 + floor(random() * 1000),
   (SELECT id FROM user_ids WHERE username = 'admin'), EXTRACT(EPOCH FROM now()) * 1000 + floor(random() * 1000)),

  -- 项目管理
  (nanoid(), '项目类型', 'project_type', 'object',
   '{"1":"研发项目","2":"运维项目","3":"咨询项目","4":"实施项目","5":"运营项目"}', (SELECT id FROM space),
   (SELECT id FROM user_ids WHERE username = 'admin'), EXTRACT(EPOCH FROM now()) * 1000 + floor(random() * 1000),
   (SELECT id FROM user_ids WHERE username = 'admin'), EXTRACT(EPOCH FROM now()) * 1000 + floor(random() * 1000)),
  (nanoid(), '项目状态', 'project_status', 'object',
   '{"1":"规划中","2":"进行中","3":"已完成","4":"已暂停","5":"已取消"}', (SELECT id FROM space),
   (SELECT id FROM user_ids WHERE username = 'admin'), EXTRACT(EPOCH FROM now()) * 1000 + floor(random() * 1000),
   (SELECT id FROM user_ids WHERE username = 'admin'), EXTRACT(EPOCH FROM now()) * 1000 + floor(random() * 1000)),
  (nanoid(), '任务状态', 'task_status', 'object',
   '{"1":"待开始","2":"进行中","3":"已完成","4":"已暂停","5":"已取消","6":"待审核"}', (SELECT id FROM space),
   (SELECT id FROM user_ids WHERE username = 'admin'), EXTRACT(EPOCH FROM now()) * 1000 + floor(random() * 1000),
   (SELECT id FROM user_ids WHERE username = 'admin'), EXTRACT(EPOCH FROM now()) * 1000 + floor(random() * 1000)),
  ...
  (nanoid(), '用户角色', 'user_role', 'object',
   ...
   EXTRACT(EPOCH FROM now()) * 1000 + floor(random() * 1000), (SELECT id FROM user_ids WHERE username = 'admin'),
   EXTRACT(EPOCH FROM now()) * 1000 + floor(random() * 1000)),
  (nanoid(), '权限类型', 'permission_type', 'object', '{"1":"菜单","2":"按钮","3":"接口"}', (SELECT id FROM space),
   (SELECT id FROM user_ids WHERE username = 'admin'), EXTRACT(EPOCH FROM now()) * 1000 + floor(random() * 1000),
   (SELECT id FROM user_ids WHERE username = 'admin'), EXTRACT(EPOCH FROM now()) * 1000 + floor(random() * 1000)),
  ...
  (nanoid(), '文档状态', 'document_status', 'object',
   '{"1":"草稿","2":"审核中","3":"已发布","4":"已归档","5":"已废弃"}', (SELECT id FROM space),
   (SELECT id FROM user_ids WHERE username = 'admin'), EXTRACT(EPOCH FROM now()) * 1000 + floor(random() * 1000),
   (SELECT id FROM user_ids WHERE username = 'admin'), EXTRACT(EPOCH FROM now()) * 1000 + floor(random() * 1000)),
  ...
  (nanoid(), '问题状态', 'issue_status', 'object',
   '{"1":"待处理","2":"处理中","3":"已解决","4":"已关闭","5":"重新打开","6":"无法重现"}', (SELECT id FROM space),
   (SELECT id FROM user_ids WHERE username = 'admin'), EXTRACT(EPOCH FROM now()) * 1000 + floor(random() * 1000),
   (SELECT id FROM user_ids WHERE username = 'admin'), EXTRACT(EPOCH FROM now()) * 1000 + floor(random() * 1000)),

  -- 资源管理
  (nanoid(), '资源类型', 'resource_type', 'object',
   '{"1":"服务器","2":"数据库","3":"存储","4":"网络设备","5":"软件许可"}', (SELECT id FROM space),
   (SELECT id FROM user_ids WHERE username = 'admin'), EXTRACT(EPOCH FROM now()) * 1000 + floor(random() * 1000),
   (SELECT id FROM user_ids WHERE username = 'admin'), EXTRACT(EPOCH FROM now()) * 1000 + floor(random() * 1000)),
  ...
  (nanoid(), '知识类型', 'knowledge_type', 'object',
   '{"1":"产品知识","2":"技术文档","3":"常见问题","4":"最佳实践","5":"故障案例"}', (SELECT id FROM space),
   (SELECT id FROM user_ids WHERE username = 'admin'), EXTRACT(EPOCH FROM now()) * 1000 + floor(random() * 1000),
   (SELECT id FROM user_ids WHERE username = 'admin'), EXTRACT(EPOCH FROM now()) * 1000 + floor(random() * 1000)),
  (nanoid(), '知识状态', 'knowledge_status', 'object',
   '{"1":"草稿","2":"待审核","3":"已发布","4":"已归档","5":"已废弃"}', (SELECT id FROM space),
   (SELECT id FROM user_ids WHERE username = 'admin'), EXTRACT(EPOCH FROM now()) * 1000 + floor(random() * 1000),
   (SELECT id FROM user_ids WHERE username = 'admin'), EXTRACT(EPOCH FROM now()) * 1000 + floor(random() * 1000)),
