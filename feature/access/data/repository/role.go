...
func (r *roleRepository) Create(ctx context.Context, body *structs.CreateRoleBody) (*ent.Role, error) {
	...
	if err != nil {
		log.Errorf(ctx, "roleRepo.Create error: %v\n", err)
		return nil, err
	}
...
func (r *roleRepository) GetByID(ctx context.Context, id string) (*ent.Role, error) {
	...
	if err != nil {
		log.Errorf(ctx, "roleRepo.GetByID error: %v\n", err)
		return nil, err
	}
	...
	if err != nil {
		log.Errorf(ctx, "roleRepo.GetByID cache error: %v\n", err)
	}
...
func (r *roleRepository) GetByIDs(ctx context.Context, ids []string) ([]*ent.Role, error) {
	...
	if err != nil {
		log.Errorf(ctx, "roleRepo.GetByIDs error: %v\n", err)
		return nil, err
	}
	return rows, nil
}
...
func (r *roleRepository) GetBySlug(ctx context.Context, slug string) (*ent.Role, error) {
	...
	if err != nil {
		log.Errorf(ctx, "roleRepo.GetBySlug error: %v\n", err)
		return nil, err
	}
	...
	err = r.c.Set(ctx, cacheKey, row)
	if err != nil {
		log.Errorf(ctx, "roleRepo.GetBySlug cache error: %v\n", err)
	}
...
func (r *roleRepository) Update(ctx context.Context, slug string, updates types.JSON) (*ent.Role, error) {
	...
	row, err := builder.Save(ctx)
	if err != nil {
		log.Errorf(ctx, "roleRepo.Update error: %v\n", err)
		return nil, err
	}
	...
	if err != nil {
		log.Errorf(ctx, "roleRepo.Update cache error: %v\n", err)
	}
...
func (r *roleRepository) List(ctx context.Context, params *structs.ListRoleParams) ([]*ent.Role, error) {
	...
	rows, err := builder.All(ctx)
	if err != nil {
		log.Errorf(ctx, "roleRepo.List error: %v\n", err)
		return nil, err
	}
...
func (r *roleRepository) Delete(ctx context.Context, slug string) error {
	...
	if _, err = builder.Where(roleEnt.IDEQ(role.ID)).Exec(ctx); err != nil {
		log.Errorf(ctx, "roleRepo.Delete error: %v\n", err)
		return err
	}
	...
	err = r.c.Delete(ctx, fmt.Sprintf("role:slug:%s", role.Slug))
	if err != nil {
		log.Errorf(ctx, "roleRepo.Delete cache error: %v\n", err)
	}
