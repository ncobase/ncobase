...
func (r *topicRepository) Create(ctx context.Context, body *structs.CreateTopicBody) (*ent.Topic, error) {
	...
	if err != nil {
		log.Errorf(ctx, "topicRepo.Create error: %v\n", err)
		return nil, err
	}

	// Create the topic in Meilisearch index
	if err = r.ms.IndexDocuments("topics", row); err != nil {
		log.Errorf(ctx, "topicRepo.Create error creating Meilisearch index: %v\n", err)
		// return nil, err
	}
...
func (r *topicRepository) GetByID(ctx context.Context, id string) (*ent.Topic, error) {
	...
	if err != nil {
		log.Errorf(ctx, "topicRepo.GetByID error: %v\n", err)
		return nil, err
	}
	...
	if err != nil {
		log.Errorf(ctx, "topicRepo.GetByID cache error: %v\n", err)
	}
...
func (r *topicRepository) GetBySlug(ctx context.Context, slug string) (*ent.Topic, error) {
	...
	if err != nil {
		log.Errorf(ctx, "topicRepo.GetBySlug error: %v\n", err)
		return nil, err
	}
	...
	err = r.c.Set(ctx, cacheKey, row)
	if err != nil {
		log.Errorf(ctx, "topicRepo.GetBySlug cache error: %v\n", err)
	}
...
func (r *topicRepository) Update(ctx context.Context, slug string, updates types.JSON) (*ent.Topic, error) {
	...
	row, err := builder.Save(ctx)
	if err != nil {
		log.Errorf(ctx, "topicRepo.Update error: %v\n", err)
		return nil, err
	}
	...
	if err != nil {
		log.Errorf(ctx, "topicRepo.Update cache error: %v\n", err)
	}

	// Update the topic in Meilisearch index
	if err = r.ms.DeleteDocuments("topics", slug); err != nil {
		log.Errorf(ctx, "topicRepo.Update error deleting Meilisearch index: %v\n", err)
		// return nil, err
	}
	if err = r.ms.IndexDocuments("topics", row); err != nil {
		log.Errorf(ctx, "topicRepo.Update error updating Meilisearch index: %v\n", err)
		// return nil, err
	}
...
func (r *topicRepository) List(ctx context.Context, params *structs.ListTopicParams) ([]*ent.Topic, error) {
	...
	if err != nil {
		log.Errorf(ctx, "topicRepo.List error: %v\n", err)
		return nil, err
	}
...
func (r *topicRepository) Delete(ctx context.Context, slug string) error {
	...
	if _, err = builder.Where(topicEnt.IDEQ(topic.ID)).Exec(ctx); err != nil {
		log.Errorf(ctx, "topicRepo.Delete error: %v\n", err)
		return err
	}
	...
	if err != nil {
		log.Errorf(ctx, "topicRepo.Delete cache error: %v\n", err)
	}

	// delete from Meilisearch index
	if err = r.ms.DeleteDocuments("topics", topic.ID); err != nil {
		log.Errorf(ctx, "topicRepo.Delete index error: %v\n", err)
		// return nil, err
	}
