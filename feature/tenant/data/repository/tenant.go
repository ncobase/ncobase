...
func (r *tenantRepository) Create(ctx context.Context, body *structs.CreateTenantBody) (*ent.Tenant, error) {
	...
	if err != nil {
		log.Errorf(ctx, "tenantRepo.Create error: %v\n", err)
		return nil, err
	}

	// Create the tenant in Meilisearch index
	if err = r.ms.IndexDocuments("tenants", row); err != nil {
		log.Errorf(ctx, "tenantRepo.Create error creating Meilisearch index: %v\n", err)
		// return nil, err
	}
...
func (r *tenantRepository) GetBySlug(ctx context.Context, id string) (*ent.Tenant, error) {
	...
	if err != nil {
		log.Errorf(ctx, " tenantRepo.GetByID error: %v\n", err)
		return nil, err
	}
	...
	if err != nil {
		log.Errorf(ctx, "tenantRepo.GetByID cache error: %v\n", err)
	}
...
func (r *tenantRepository) GetByUser(ctx context.Context, userID string) (*ent.Tenant, error) {
	...
	if err != nil {
		log.Errorf(ctx, " tenantRepo.GetByUser error: %v\n", err)
		return nil, err
	}
	...
	err = r.c.Set(ctx, cacheKey, row)
	if err != nil {
		log.Errorf(ctx, "tenantRepo.GetByUser cache error: %v\n", err)
	}
...
func (r *tenantRepository) GetIDByUser(ctx context.Context, userID string) (string, error) {
	...
	if err != nil {
		log.Errorf(ctx, " tenantRepo.FindTenantIDByUserID error: %v\n", err)
		return "", err
	}
...
func (r *tenantRepository) Update(ctx context.Context, slug string, updates types.JSON) (*ent.Tenant, error) {
	...
	row, err := builder.Save(ctx)
	if err != nil {
		log.Errorf(ctx, "tenantRepo.Update error: %v\n", err)
		return nil, err
	}
	...
	if err != nil {
		log.Errorf(ctx, "tenantRepo.Update cache error: %v\n", err)
	}
	...
	if err = r.ms.UpdateDocuments("topics", row, row.ID); err != nil {
		log.Errorf(ctx, "tenantRepo.Update error updating Meilisearch index: %v\n", err)
	}
...
func (r *tenantRepository) List(ctx context.Context, params *structs.ListTenantParams) ([]*ent.Tenant, error) {
	...
	if err != nil {
		log.Errorf(ctx, " tenantRepo.GetTenantList error: %v\n", err)
		return nil, err
	}
...
func (r *tenantRepository) Delete(ctx context.Context, id string) error {
	...
	if _, err = builder.Where(tenantEnt.IDEQ(id)).Exec(ctx); err == nil {
		log.Errorf(ctx, "tenantRepo.Delete error: %v\n", err)
		return err
	}
	...
	err = r.c.Delete(ctx, cacheKey)
	if err != nil {
		log.Errorf(ctx, "tenantRepo.Delete cache error: %v\n", err)
	}

	// delete from Meilisearch index
	if err = r.ms.DeleteDocuments("tenants", tenant.ID); err != nil {
		log.Errorf(ctx, "tenantRepo.Delete index error: %v\n", err)
		// return nil, err
	}
...
func (r *tenantRepository) DeleteByUser(ctx context.Context, userID string) error {
	...
	if _, err := builder.Where(tenantEnt.CreatedByEQ(userID)).Exec(ctx); err == nil {
		log.Errorf(ctx, "tenantRepo.DeleteByUser error: %v\n", err)
		return err
	}
	...
	err := r.c.Delete(ctx, cacheUserKey)
	if err != nil {
		log.Errorf(ctx, "tenantRepo.DeleteByUser cache error: %v\n", err)
	}

	// delete from Meilisearch index
	if err = r.ms.DeleteDocuments("tenants", userID); err != nil {
		log.Errorf(ctx, "tenantRepo.DeleteByUser index error: %v\n", err)
		// return nil, err
	}
