...
func (r *roleRepo) Create(ctx context.Context, body *structs.CreateRoleBody) (*ent.Role, error) {
	...
	if err != nil {
		log.Errorf(nil, "roleRepo.Create error: %v\n", err)
		return nil, err
	}
...
func (r *roleRepo) GetByID(ctx context.Context, id string) (*ent.Role, error) {
	...
	if err != nil {
		log.Errorf(nil, "roleRepo.GetByID error: %v\n", err)
		return nil, err
	}
	...
	if err != nil {
		log.Errorf(nil, "roleRepo.GetByID cache error: %v\n", err)
	}
...
func (r *roleRepo) GetBySlug(ctx context.Context, slug string) (*ent.Role, error) {
	...
	if err != nil {
		log.Errorf(nil, "roleRepo.GetBySlug error: %v\n", err)
		return nil, err
	}
	...
	err = r.c.Set(ctx, cacheKey, row)
	if err != nil {
		log.Errorf(nil, "roleRepo.GetBySlug cache error: %v\n", err)
	}
...
func (r *roleRepo) Update(ctx context.Context, slug string, updates types.JSON) (*ent.Role, error) {
	...
	row, err := builder.Save(ctx)
	if err != nil {
		log.Errorf(nil, "roleRepo.Update error: %v\n", err)
		return nil, err
	}
	...
	if err != nil {
		log.Errorf(nil, "roleRepo.Update cache error: %v\n", err)
	}
...
func (r *roleRepo) List(ctx context.Context, p *structs.ListRoleParams) ([]*ent.Role, error) {
	...
	if err != nil {
		log.Errorf(nil, "roleRepo.List error: %v\n", err)
		return nil, err
	}
...
func (r *roleRepo) Delete(ctx context.Context, slug string) error {
	...
	if _, err = builder.Where(roleEnt.IDEQ(role.ID)).Exec(ctx); err != nil {
		log.Errorf(nil, "roleRepo.Delete error: %v\n", err)
		return err
	}
	...
	err = r.c.Delete(ctx, fmt.Sprintf("role:slug:%s", role.Slug))
	if err != nil {
		log.Errorf(nil, "roleRepo.Delete cache error: %v\n", err)
	}
