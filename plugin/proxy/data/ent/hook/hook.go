...
// The TransformerFunc type is an adapter to allow the use of ordinary
// function as Transformer mutator.
type TransformerFunc func(context.Context, *ent.TransformerMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f TransformerFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.TransformerMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.TransformerMutation", m)
}
