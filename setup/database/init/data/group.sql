...
WITH tenant AS
     ...
     group_3 AS (
       INSERT
         INTO ncse_org_group (id, name, slug, parent_id, tenant_id, created_by, updated_by, created_at, updated_at)
           VALUES (nanoid(), '策划组', 'planning-team',
     ...
     group_4 AS (
       INSERT
         INTO ncse_org_group (id, name, slug, parent_id, tenant_id, created_by, updated_by, created_at, updated_at)
           VALUES (nanoid(), '会计组', 'accounting-team',
                   (SELECT id FROM department_1 WHERE slug = 'finance-department'), (SELECT id FROM tenant),
                   (SELECT id FROM user_ids WHERE username = 'admin'),
                   (SELECT id FROM user_ids WHERE username = 'admin'),
                   EXTRACT(EPOCH FROM now()) * 1000 + floor(random() * 1000),
                   EXTRACT(EPOCH FROM now()) * 1000 + floor(random() * 1000)),
