-- 插入分类数据
WITH space AS
         (SELECT id FROM ncse_sys_space LIMIT 1),
     user_ids AS
       (SELECT id, username
        FROM ncse_sys_user),
     taxonomy_data_1 AS (
         ...
         INTO ncse_cms_taxonomy (id, name, type, slug, cover, thumbnail, color, icon, url, keywords, description,
           ...
           VALUES
             ...
             (nanoid(), '技术', 'node', 'jishu', 'cover_tech.jpg', 'thumb_tech.jpg', '#0000FF', 'icon_tech.png',
              'https://ncobase.com/tech', '技术，创新', '与技术相关的所有内容。', 0, '{
               "extra_info": "技术类别"
             }', NULL, (SELECT id FROM space), (SELECT id FROM user_ids WHERE username = 'admin'),
             ...
             (nanoid(), '科学', 'node', 'kexue', 'cover_sci.jpg', 'thumb_sci.jpg', '#00FF00', 'icon_sci.png',
              'https://ncobase.com/science', '科学，研究', '科学主题和研究。', 0, '{
               "extra_info": "科学类别"
             }', NULL, (SELECT id FROM space), (SELECT id FROM user_ids WHERE username = 'admin'),
              (SELECT id FROM user_ids WHERE username = 'admin'),
              EXTRACT(EPOCH FROM now()) * 1000 + floor(random() * 1000),
             ...
             (nanoid(), '营销', 'node', 'yingxiao', 'cover_marketing.jpg', 'thumb_marketing.jpg', '#FFD700',
              'icon_marketing.png', 'https://ncobase.com/marketing', '营销，策略', '营销和策略。', 0, '{
               "extra_info": "营销类别"
             }', NULL, (SELECT id FROM space), (SELECT id FROM user_ids WHERE username = 'admin'),
              (SELECT id FROM user_ids WHERE username = 'admin'),
              EXTRACT(EPOCH FROM now()) * 1000 + floor(random() * 1000),
         ...
         INTO ncse_cms_taxonomy (id, name, type, slug, cover, thumbnail, color, icon, url, keywords, description,
           ...
           VALUES
             ...
             (nanoid(), '编程', 'node', 'biancheng', 'cover_prog.jpg', 'thumb_prog.jpg', '#FF0000', 'icon_prog.png',
              'https://ncobase.com/programming', '编程，开发', '编程语言和开发。', 0, '{
               "extra_info": "编程子类别"
             }', (SELECT id FROM taxonomy_data_1 WHERE name = '技术'), (SELECT id FROM space),
             ...
             (nanoid(), '人工智能', 'node', 'rengongzhineng', 'cover_ai.jpg', 'thumb_ai.jpg', '#FFAAAA', 'icon_ai.png',
              'https://ncobase.com/ai', '人工智能，智能', '人工智能技术和应用。', 0, '{
               "extra_info": "人工智能子类别"
             }', (SELECT id FROM taxonomy_data_1 WHERE name = '技术'), (SELECT id FROM space),
              (SELECT id FROM user_ids WHERE username = 'admin'), (SELECT id FROM user_ids WHERE username = 'admin'),
              EXTRACT(EPOCH FROM now()) * 1000 + floor(random() * 1000),
             ...
             (nanoid(), '数据科学', 'node', 'shujukexue', 'cover_ds.jpg', 'thumb_ds.jpg', '#00FFFF', 'icon_ds.png',
              'https://ncobase.com/datascience', '数据科学，分析', '数据科学和分析。', 0, '{
               "extra_info": "数据科学子类别"
             }', (SELECT id FROM taxonomy_data_1 WHERE name = '科学'), (SELECT id FROM space),
              (SELECT id FROM user_ids WHERE username = 'admin'), (SELECT id FROM user_ids WHERE username = 'admin'),
              EXTRACT(EPOCH FROM now()) * 1000 + floor(random() * 1000),
              EXTRACT(EPOCH FROM now()) * 1000 + floor(random() * 1000)),
             -- 战略规划 (项目管理的子类别)
             (nanoid(), '战略规划', 'node', 'zhanlueguihua', 'cover_strategy.jpg', 'thumb_strategy.jpg', '#8A2BE2',
              'icon_strategy.png', 'https://ncobase.com/strategy', '战略规划，发展', '战略规划与发展。', 0, '{
               "extra_info": "战略规划子类别"
             }', (SELECT id FROM taxonomy_data_1 WHERE name = '项目管理'), (SELECT id FROM space),
              (SELECT id FROM user_ids WHERE username = 'admin'), (SELECT id FROM user_ids WHERE username = 'admin'),
              EXTRACT(EPOCH FROM now()) * 1000 + floor(random() * 1000),
              EXTRACT(EPOCH FROM now()) * 1000 + floor(random() * 1000)),
             -- 广告 (营销的子类别)
             (nanoid(), '广告', 'node', 'guanggao', 'cover_ad.jpg', 'thumb_ad.jpg', '#FF6347', 'icon_ad.png',
              'https://ncobase.com/advertising', '广告，宣传', '广告与宣传。', 0, '{
               "extra_info": "广告子类别"
             }', (SELECT id FROM taxonomy_data_1 WHERE name = '营销'), (SELECT id FROM space),
              (SELECT id FROM user_ids WHERE username = 'admin'), (SELECT id FROM user_ids WHERE username = 'admin'),
              EXTRACT(EPOCH FROM now()) * 1000 + floor(random() * 1000),
              EXTRACT(EPOCH FROM now()) * 1000 + floor(random() * 1000)),
             -- 市场调研 (营销的子类别)
             (nanoid(), '市场调研', 'node', 'shichangdiaocha', 'cover_market_research.jpg', 'thumb_market_research.jpg',
              '#98FB98', 'icon_market_research.png', 'https://ncobase.com/marketresearch', '市场调研，分析',
              '市场调研和分析。', 0, '{
               "extra_info": "市场调研子类别"
             }', (SELECT id FROM taxonomy_data_1 WHERE name = '营销'), (SELECT id FROM space),
              (SELECT id FROM user_ids WHERE username = 'admin'), (SELECT id FROM user_ids WHERE username = 'admin'),
         ...
         INTO ncse_cms_taxonomy (id, name, type, slug, cover, thumbnail, color, icon, url, keywords, description,
           ...
           VALUES
             ...
             (nanoid(), '机器学习', 'node', 'jishixuexi', 'cover_ml.jpg', 'thumb_ml.jpg', '#FF00FF', 'icon_ml.png',
              'https://ncobase.com/ml', '机器学习，人工智能', '机器学习和人工智能。', 0, '{
               "extra_info": "机器学习子类别"
             }', (SELECT id FROM taxonomy_data_2 WHERE name = '人工智能'), (SELECT id FROM space),
              (SELECT id FROM user_ids WHERE username = 'admin'), (SELECT id FROM user_ids WHERE username = 'admin'),
              EXTRACT(EPOCH FROM now()) * 1000 + floor(random() * 1000),
              EXTRACT(EPOCH FROM now()) * 1000 + floor(random() * 1000)),
             -- 大数据 (数据科学的子类别)
             (nanoid(), '大数据', 'node', 'dadata', 'cover_bd.jpg', 'thumb_bd.jpg', '#AAFFAA', 'icon_bd.png',
              'https://ncobase.com/bigdata', '大数据，分析', '大数据技术和应用。', 0, '{
               "extra_info": "大数据子类别"
             }', (SELECT id FROM taxonomy_data_2 WHERE name = '数据科学'), (SELECT id FROM space),
              (SELECT id FROM user_ids WHERE username = 'admin'), (SELECT id FROM user_ids WHERE username = 'admin'),
              EXTRACT(EPOCH FROM now()) * 1000 + floor(random() * 1000),
              EXTRACT(EPOCH FROM now()) * 1000 + floor(random() * 1000))
           RETURNING id, name),
